...
```{r CFA partial bi-factor model de Bruin, eval=FALSE, include=FALSE}
...
```{r CFA three factor model with correlated errors Dominguez, include=FALSE}
...
```{r CFA replicated_errors_model, include=FALSE}
...
lavaan::anova(UWES_replicated_errors,UWES_combined_modified)
```
...
A series of Confirmatory Factor Analyses (CFA) were conducted to determine the optimal factorial structure of the UWES-9 for the Czech sample. As noted in the methods, initial models based on standard one-factor, two-factor, hierarchical, and correlated three-factor structures demonstrated poor fit to the data. For instance, the standard three-factor model yielded an RMSEA of `r round(fitMeasures(UWES.three.fac)['rmsea'], 3)`, which was substantially higher than its dynamically-generated Level-0 cutoff of `r round(DFI_three_factor$cutoffs['RMSEA', 'Level-0'], 3)`, indicating a significant degree of misfit and the need for a more nuanced model.

This led to the testing of modified three-factor models that account for local misfit. Two primary models were retained for a final comparison. The first, a **"Replicated Errors" model**, was specified based on the most consistent findings in the international literature. This model included correlated errors between items 1 and 2 (Vigor) and items 8 and 9 (Absorption), a modification independently identified across several validation studies (e.g., Domínguez-Salas et al., 2022; Simbula et al., 2013). The second, a **"Three-Factor Model with Within-Factor Correlated Errors"**, included the same two correlations plus an additional error covariance between items 3 and 4 (Dedication). This more complex model was tested to confirm whether any remaining misfit in the "Replicated Errors" model could be systematically explained by this additional theoretically-grounded correlation (Zecca et al., 2015).

To adjudicate between these nested models, a scaled chi-square difference test was performed. The test revealed no significant improvement in fit for the more complex model with three correlated errors (*χ²*diff(`r lavaan::anova(UWES_replicated_errors, UWES_within_factor_corr, robust = TRUE)$'Df diff'[2]`) = `r format(round(lavaan::anova(UWES_replicated_errors, UWES_within_factor_corr, robust = TRUE)$'Chisq diff'[2], 2), nsmall = 2)`, *p* = `r format(round(lavaan::anova(UWES_replicated_errors, UWES_within_factor_corr, robust = TRUE)$'Pr(>Chisq)'[2], 2), nsmall = 2)`). Based on the principle of parsimony, the simpler "Replicated Errors" model was retained as the final and most appropriate model.

The fit of this final model was evaluated against dynamically generated fit index cutoffs. The model produced an excellent CFI (`r format(round(fitMeasures(UWES_replicated_errors)['cfi'], 3), nsmall = 3)`) and a low SRMR (`r format(round(fitMeasures(UWES_replicated_errors)['srmr'], 3), nsmall = 3)`). However, the RMSEA (`r format(round(fitMeasures(UWES_replicated_errors)['rmsea'], 3), nsmall = 3)`) was considerably higher than its dynamically generated Level-0 cutoff of `r format(round(DFI_replicated_errors$cutoffs['RMSEA', 'Level-0'], 3), nsmall = 3)`, indicating that while the model is superior to all tested alternatives, it still contains a non-trivial degree of misfit. This suggests that while this three-factor structure with modifications for item wording overlap is the best representation of the data, the UWES-9 scale may not perfectly capture the construct of work engagement within this sample. A summary of the fit indices for all tested models is provided in Table [Insert Table Number Here].
...
semPaths(UWES.three.fac,layout = "tree2", whatLabels= "std",
         ...
         intercepts = F, rotation = 1, filetype = c("eps"), filename = "Figure.1",  
...
DFI_table = create_dynamic_table(
  # c(
  #   "DFI_one_factor",
  #   "DFI_two_factor",
  #   "DFI_three_factor",
  #   "DFI_three_factor_valenced",
  #   "DFI_five_factor",
  #   "DFI_modified_five_factor"
  # ),
  add_interpretation = FALSE,
  show_fitted_model = TRUE,
  add_separator_rows = FALSE,
  ...
  # mutate(
  #   Model = case_when(
  #     Model == "five_factor" ~ "Five factor",
  #     Model == "modified_five_factor" ~ "Modified five factor",
  #     Model == "one_factor" ~ "One factor",
  #     Model == "three_factor" ~ "Three factor",
  #     Model == "three_factor_valenced" ~ "Three factor valenced",
  #     Model == "two_factor" ~ "Two factor",
  #     TRUE ~ Model  # Keep other values unchanged (like NA)
  #   )
  # )
...
```{r internal consistency examination, message=FALSE, warning=FALSE, include=FALSE}
# UWES total
...
mult.group.m = eqMI.main(model = meas.invar.UWES,
                         ...
                         adjRMSEA = T,
                         projection = T,
                         ordered = FALSE) # in previous version of the script this was set as TRUE but due to the following error: "Error: lavaan->lavTestLRT():  
   #method = “satorra.bentler.2001” only available when models are fitted with test = "satorra.bentler", "yuan.bentler", or "yuan.bentler.mplus""; this needed to
#be modified
