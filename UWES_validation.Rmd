...
Breusch-Pagan test ($\chi^2$ = `r round(bp.t.1$ChiSquare,digits=2)`, df = `r bp.t.1$Df`, `r format_p(bp.t.1$p)`).




 Therefore, statistical test not assuming Gaussian distribution were used. Visual screening of residual plot suggested slight heteroscedasticity (Sample 1), which was not supported by Breusch-Pagan test ($\chi^2$ = `r round(bp.t.1$ChiSquare,digits=2)`, *df* = `r bp.t.1$Df`, `r format_p(bp.t.1$p)`).  
  
  To explore the dimensionality of the OASIS, Confirmatory Factor Analysis (CFA) on the first sample (*n* = `r nrow(data)`) was performed. As the OASIS was developed as a unidimensional scale, only one factor model was evaluated. Power analysis suggested that for the purposes of the CFA, at least 1021 participants is needed. More details about power analysis procedure can be found in preregistration form (https://osf.io/yxm9g). Adequacy of correlation matrix was assessed by Bartlett test and also by Kaiser Meyer Olkin (KMO) measure. Absolute model fit was explored by the following indicators: Standardized Root Mean Square Residual (SRMR) and by Root Mean Square Error of Approximation (RMSEA). In these indicators, values < 0.08 suggest acceptable fit and < 0.05 a good fit [@civelek2018essentials; @hoe_issues_2008; @vandenberg_review_2000; @hooper_structural_2008]. The Comparative Fit index (CFI) and Tucker-Lewis index (TLI) were used to evaluate incremental fit of the model. Values of the TLI and CFI > 0.95 suggest an acceptable fit [@jackson_reporting_2009] and values > 0.97 a good fit [@schermelleh_engel_evaluating_2003]. Model implied and observed covariance was compared by the chi-square test. As manifest variables have five categories, based on recommendation of @distefano_comparison_2014, we used Diagonally Weighted Least Squares estimator (DWLS) to fit our model. Polychoric correlations were utilized during fitting procedure. 
  
  Measurement equivalence was tested via multigroup CFA. Invariance of the measurement was explored in gender (males,famales) and also in clinical and non - clinical population. The first equivalence analysis was conducted on the Sample 1, the second analysis on the Sample 2. If $\Delta$ CFA was > 0.01, than good fit of a nested model was rejected. Internal consistency of OASIS was assessed by Cronbach’s alpha and  McDonald’s omega. Convergent, divergent and concurent validity was inspected by zero - order Spearman correlation coefficient and by analysis of covariance (ANCOVA). In ANCOVA (Sample 2), outcome variable was gender, predictor the OASIS score and neuroticism as a covariate. Correlation analysis (Sample 2, 3), was conducted to verify H3 - H14. Temporal stability of the OASIS score will be examined by two-way random effects intracalss correlation coefficient. Time interval between the first and the second administration of the OASIS was one week (Sample 2). Evaluation of the cutt-of points was conducted using Receiver Operating Characteristics (ROC) analysis. Youden's index was calculated to find out optimal compromise between sensitivity and specificity. The ROC and the Youden's index calculation was done on the Sample 2.    
  
  Differences between socio-demographic groups in the OASIS (Sample 1) were examined via Wilcoxon signed rank test and by Kruskal–Wallis test (non-parametric ANOVA). Post-hoc analysis will consist of Dunn test and Games-Howell test. Degree of an effect for socio-demographic comparison was estimated by Vargha and Delaney $\hat{A}$ [@vargha_critique_2000], in which values of $\hat{A}$ between 0.56 - 0.64 indicates small effect; 0.64 - 0.71 medium effect and > 0.71 a large effect. Descriptive and inferential statistics procedures were done in the `r cite_r()` using the following libraries: *lavaan* [@R-lavaan], *papaja* [@R-papaja] *psych* [@R-psych], *usf* [@R-ufs].     
...
# OASIS
# Gender
car::leveneTest(as.numeric(data$OASIS) ~ data$Gender, data = data, center = mean) # heteroscedasticity
# Education
car::leveneTest(as.numeric(data$OASIS) ~ data$Education, data = data, center = mean) # heteroscedasticity
# Religiosity
car::leveneTest(as.numeric(data$OASIS) ~ data$Religiosity, data = data, center = mean) # heteroscedasticity
# Socio-economic status
car::leveneTest(as.numeric(data$OASIS) ~ data$Economical_status, data = data, center = mean) # heteroscedasticity
# Family status
car::leveneTest(as.numeric(data$OASIS) ~ data$Family_status, data = data, center = mean) # heteroscedasticity
# Gender
car::leveneTest(as.numeric(data$OASIS) ~ data$Gender, data = data, center = mean) # heteroscedasticity
#....................................................................................................................
...
```{r CFA_OASIS_cor_er_it_1_2, include=FALSE}
# One factor model 
one.fac.mod.cor.er.oasis="Anxiety =~ OASIS_1 + OASIS_2 + OASIS_3 + OASIS_4 + OASIS_5 
             OASIS_1 ~~ OASIS_2"
...
OASIS.one.dim.oasis.cor.eror=sem(model = one.fac.mod.cor.er.oasis, data = data, std.lv=T, ordered = T)
...
meas.invar.oasis="Anxiety =~ OASIS_1 + OASIS_2 + OASIS_4 + OASIS_5 # there was a need to remove item 3 from the model as the some categories were empthy in one group  
                             OASIS_1 ~~ OASIS_2"
...
variab.cor.data.ex=data.ex[, c("OASIS","ODSIS","Gender","BFI_N","RSES","PANAS_P","PANAS_N","Age","education","faith")]
# recode variables to numeric
