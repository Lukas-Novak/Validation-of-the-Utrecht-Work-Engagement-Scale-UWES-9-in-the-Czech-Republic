...
packages=c("psych","dplyr","lavaan","base","foreign","Routliers","MVN","parameters", "EFA.MRFA","RGenData","semPlot","psycho","apa","corx","effectsize","qwraps2","finalfit","ggstatsplot","dunn.test","devtools","rticles","ICC.Sample.Size","tidyverse","magrittr","osfr","expss","gtsummary","huxtable","equaltestMI","haven","rcompanion","ufs","semTools","semPlot","insight","flextable","officer","officedown") 
...
close_section <- prop_section(
    page_size = page_size(orient = "landscape", width = 12, height = 12), 
    type = "continuous")
```
...
cr_values <- compRelSEM(UWES.three.fac.corr.errors.Dominguez)

# Calculate Average Variance Extracted (AVE)
ave_values <- AVE(UWES.three.fac.corr.errors.Dominguez)
...
Bartlett test ($\chi^2$ (`r bartlett.UWES$df`) = `r round(bartlett.UWES$chisq,digits=2)`, `r format_p(bartlett.UWES$p.value)`) as well as KMO (`r round(KMO(UWES.rep.poly)$MSA,digits = 2)`) revealed that UWES data are sufficiently correlated to perform factor analysis. Next, a series of CFAs were conducted to determine the optimal factorial structure of the UWES-9 for the Czech sample. Results of these analysis are presented in the Table \@ref(tab:DFITable). Initial models based on standard one-factor, two-factor, hierarchical, and correlated three-factor structures demonstrated poor fit to the data. For instance, the standard three-factor model yielded an RMSEA of `r round(fitMeasures(UWES.three.fac)['rmsea.scaled'], 3)`, which was substantially higher than its dynamically-generated Level-0 cutoff of `r round(as.numeric(DFI_three_factor$cutoffs['Level-0', 'RMSEA']), 3)`, indicating a significant degree of misfit and the need for a more nuanced model.

This led to the testing of modified three-factor models to account for sources of local misfit. Two primary models were retained for a final, decisive comparison. The first, based on the work of **Domínguez-Salas et al. (2022)**, was specified based on the most consistent findings in the international literature. This model included correlated errors between items 1 and 2 (Vigor) and items 8 and 9 (Absorption), a modification independently identified across several validation studies.

The second model, following **Zecca et al. (2015)**, included the same two correlations plus an additional error covariance between items 3 and 4 (Dedication). This more complex model was tested to confirm whether any remaining misfit in the Domínguez-Salas et al. (2022) model could be systematically explained by this additional theoretically-grounded correlation. The purpose of this test was to ensure that the most parsimonious model was selected only after confirming that a more complex—but still justifiable—model did not provide a statistically superior explanation of the data.

To adjudicate between these nested models, a scaled chi-square difference test was performed. The test revealed no significant improvement in fit for the more complex model with three correlated errors ($\chi^2$diff(`r lavaan::anova(UWES.three.fac.corr.errors.Dominguez, uwes_three_fac_zecca, robust = TRUE)$'Df diff'`) = `r format(round(lavaan::anova(UWES.three.fac.corr.errors.Dominguez, uwes_three_fac_zecca, robust = TRUE)$'Chisq diff', 2), nsmall = 2)`, *p* = `r format(round(lavaan::anova(UWES.three.fac.corr.errors.Dominguez, uwes_three_fac_zecca, robust = TRUE)$'Pr(>Chisq)', 2), nsmall = 2)`). Based on the principle of parsimony, the simpler model from **Domínguez-Salas et al. (2022)** was selected as the final and most appropriate model.

The fit of this final model was evaluated against dynamically generated fit index cutoffs. The model produced an excellent CFI (`r format(round(fitMeasures(UWES.three.fac.corr.errors.Dominguez)['cfi.scaled'], 3), nsmall = 3)`) and a low SRMR (`r format(round(fitMeasures(UWES.three.fac.corr.errors.Dominguez)['srmr'], 3), nsmall = 3)`). However, the RMSEA (`r format(round(fitMeasures(UWES.three.fac.corr.errors.Dominguez)['rmsea.scaled'], 3), nsmall = 3)`) was considerably higher than its dynamically generated Level-0 cutoff of `r format(round(as.numeric(DFI_three_factor_corr_errors_Dominguez$cutoffs['Level-0', 'RMSEA']), 3), nsmall = 3)`, indicating that while the model is superior to all tested alternatives, it still contains a non-trivial degree of misfit. This suggests that while this three-factor structure with modifications for item wording overlap is the best representation of the data, the UWES-9 scale may not perfectly capture the construct of work engagement within this sample.

To further evaluate the psychometric properties of this final model, we examined its construct validity. The model demonstrated excellent reliability and convergent validity at the factor level. Composite Reliability (CR) for the Vigor, Dedication, and Absorption factors were `r cr_vigor`, `r cr_dedication`, and `r cr_absorption`, respectively, all well above the recommended 0.70 threshold. Similarly, the Average Variance Extracted (AVE) for each factor was high (Vigor = `r ave_vigor`; Dedication = `r ave_dedication`; Absorption = `r ave_absorption`), comfortably exceeding the 0.50 benchmark for convergent validity.

However, the high inter-factor correlations (see Figure \@ref(fig:my-figure)) prompted a critical test of discriminant validity. We calculated the Heterotrait-Monotrait Ratio of Correlations (HTMT) to assess the distinctness of the three factors. The results showed a ratio of `r htmt_vd` between Vigor and Dedication, `r htmt_va` between Vigor and Absorption, and `r htmt_da` between Dedication and Absorption. As all values substantially exceeded the 0.90 threshold, the analysis indicates that the three factors, while structurally sound, are not empirically distinct constructs in our sample.

The finding that a three-factor model provides the best statistical fit, while its factors simultaneously lack discriminant validity, presents a nuanced interpretive challenge. This indicates that while the UWES items do form three statistically distinguishable clusters as specified by the instrument's theory, these factors share an overwhelming degree of conceptual variance. The extremely high HTMT ratios suggest that, in practice, the factors represent closely related facets of a single underlying work engagement construct rather than functionally independent dimensions. Thus, for further analyses, we used UWES total score rather than invididual subscales score.

<!-- Therefore, while the three-factor structure is retained as the best-fitting measurement model, using the individual subscale scores as distinct variables in subsequent analyses (e.g., regressions) would be problematic. The profound empirical overlap would make it difficult to disentangle their unique predictive effects and could lead to unstable results. For analyses where the primary goal is to understand the relationship between overall work engagement and other outcomes, a total score provides a more stable, parsimonious, and conceptually clear measure. This approach honors the superior statistical fit of the three-factor architecture while acknowledging the practical and conceptual implications of the factors' lack of distinctiveness. -->
...
DFI_table_publication_final %>%
  ...
  apa_table(col.names = c(" ", 
                         ...
                         " ", " ", " ", " ", " ",
                         " ", " ", " ", " "),
            caption = "Confirmatory Factor Analysis Fit Indices for Alternative Model Specifications", 
            note = "SRMR = Standardized Root Mean Square Residual; RMSEA = Root Mean Square Error of Approximation; CFI = Comparative Fit Index. Bold values indicate the fitted model results. NONE indicates parameter was not estimated for that specification level.")
...
op_section <- prop_section(type = "continuous")
close_section <- prop_section(
  page_size = page_size(orient = "landscape", width = 10, height = 8), 
  type = "continuous")
```
...
Results of the regression analysis revealed that work engagement is significantly related with chronic diseases. Specifically, higher work engagement was significantly related with lower probability of developing skin diseases or eczema (in crude effect) pain of unclear origin (both crude and adjusted effect see \@ref(tab:LogRegChronicDiseas)). 
